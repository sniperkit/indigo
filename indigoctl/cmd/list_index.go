...
func runEListIndexCmd(cmd *cobra.Command, args []string) error {
	protoListIndexRequest := &proto.ListIndexRequest{}

	conn, err := grpc.Dial(listCmdOpts.gRPCServer, grpc.WithInsecure())
	if err != nil {
		return err
	}
	defer conn.Close()
	...
	resp, err := client.ListIndex(context.Background(), protoListIndexRequest)
	if err != nil {
		return err
	}
